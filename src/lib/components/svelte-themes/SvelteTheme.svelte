<script lang="ts">
  ...
  themeStore.set({
    theme: initialTheme,
    forcedTheme,
    resolvedTheme: getResolvedTheme(initialTheme, themes),
  ...
  const changeTheme = (theme, updateStorage = true, updateDOM = true) => {
    ...
    themeStore.update((s) => {
      s.resolvedTheme = getResolvedTheme(theme, themes);
      return s;
    });
  };
  ...
  const mediaHandler = (e?) => {
    ...
    $themeStore.resolvedTheme = getResolvedTheme($themeStore.theme, themes);
  ...
  $: {
    if (forcedTheme) {
      changeTheme(forcedTheme, true, false);
    } else {
      changeTheme(theme);
    }
  }
